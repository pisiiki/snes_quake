...
BRUSH_REPLAY_SAMPLE_RATE_HZ = 30
...
def encode_brush_replay(replay: CanonicalBrushReplay) -> bytes:
    """Pack full active sets while deduplicating identical 30 Hz snapshots."""
    if not replay.rows:
        raise ValueError("cannot encode an empty brush replay")
    if replay.sample_rate_hz != BRUSH_REPLAY_SAMPLE_RATE_HZ:
        raise ValueError(
            f"brush replay sample rate must be {BRUSH_REPLAY_SAMPLE_RATE_HZ} Hz"
        )
...
def decode_brush_replay(data: bytes) -> DecodedBrushReplay:
    ...
    if (
        rate != BRUSH_REPLAY_SAMPLE_RATE_HZ
