...
IMMEDIATE_MOVE_PATTERN = re.compile(
    r"^\s*move\s+r(?:1[0-5]|[0-9])\s*,\s*#.+$",
    re.IGNORECASE,
)
...
def _unsafe_delay_slot_operand_bytes(text: str, opcode: str) -> str | None:
    """Name statements whose opcode needs bytes after a taken transfer."""
    if opcode in OPERAND_BYTE_DELAY_OPCODES:
        return opcode.upper()
    code = _strip_comment(text).strip()
    while True:
        label = LABEL_PATTERN.match(code)
        if not label:
            break
        code = code[label.end() :]
    return "MOVE" if IMMEDIATE_MOVE_PATTERN.fullmatch(code) else None
...
def _scan(
    ...
    for number, text in enumerate(
        ...
        if delay_slot_branch:
            ...
            operand_opcode = _unsafe_delay_slot_operand_bytes(code, opcode)
            if operand_opcode:
                branch_line, branch_opcode = delay_slot_branch
                violations.append(
                    Fx3OpcodeViolation(
                        ...
                        number,
                        operand_opcode,
                        f"{operand_opcode} needs operand bytes after its opcode "
                        f"in the {branch_opcode.upper()} delay slot from line "
                        f"{branch_line}; a taken transfer fetches those bytes "
                        "from its target, so use a one-byte encoding",
                    )
                )
        delay_slot_branch = (
        ...
        if reason:
            violations.append(Fx3OpcodeViolation(path, number, opcode.upper(), reason))
...
def source_targets_fx3(source: Path) -> bool:
    for text in source.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if (
            include
            and Path(include.group(1).replace("\\", "/")).name.lower() == FX3_INCLUDE
        ):
            return True
    return False
