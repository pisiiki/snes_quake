...
def parse_ld65_labels(
    text: str,
    *,
    source: str = "label text",
...
def load_required_ld65_labels(path: Path, required: Iterable[str]) -> dict[str, int]:
    """Load only requested labels while tolerating unrelated address-space clashes."""
    label_path = Path(path)
    names = tuple(required)
    labels = parse_ld65_labels(
        label_path.read_text(encoding="utf-8"),
        source=str(label_path),
        names=names,
    )
    return require_ld65_labels(labels, names, source=str(label_path))
