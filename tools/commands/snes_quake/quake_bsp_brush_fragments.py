...
def clip_brush_state_to_world(
    source: Any, root: int, state: BrushEntityState
) -> tuple[Fragment, ...]:
    """Derive one translated inline-model pose through the static world BSP."""

    if state.angles != (0.0, 0.0, 0.0):
        raise ValueError("BSP-derived brush fragments require translation-only poses")
    model = source.models[state.inline_model]
    output: list[Fragment] = []
    for source_face in range(int(model[14]), int(model[14]) + int(model[15])):
        polygon = tuple(
            tuple(
                source.vertices[vertex][axis] + state.origin[axis] for axis in range(3)
            )
            for vertex in _face_vertex_ids(source, source.faces[source_face])
        )
        output.extend(_clip_face_to_world(source, root, source_face, polygon))
    return tuple(output)
...
def audit_leaf_clipped_fragments(
    ...
    """Measure a separate fragment pool over every configured 2 Hz step row."""
    ...
    for row_index in step_row_indices(len(replay.rows), replay.sample_rate_hz):
    ...
    return {
        ...
        "stepRowsFormula": "row = n * sampleRateHz / 2",
...
def _runtime_axis_coordinate_q4(
    ...
    coordinate = vertex_q2[axis] * 64 + world_origin_q0[axis] * 16 - origin_q3[axis] * 2
    if axis_code < 0:
...
def _pack_control_tables(
    tables: tuple[tuple[str, bytes], ...],
) -> tuple[list[bytearray], dict[str, tuple[int, int]], int]:
    """Place each fixed-address control table wholly within one ROM bank."""

    chunks = [bytearray(b"\xff" * ROM_BANK_BYTES)]
    locations: dict[str, tuple[int, int]] = {}
    offset = FRAGMENT_HEADER.size
    logical_bytes = FRAGMENT_HEADER.size
    for name, payload in tables:
        if len(payload) > ROM_BANK_BYTES:
            raise ValueError(
                f"brush fragment {name} table needs {len(payload)} bytes; "
                f"one-table bound is {ROM_BANK_BYTES}"
            )
        if offset + len(payload) > ROM_BANK_BYTES:
            chunks.append(bytearray(b"\xff" * ROM_BANK_BYTES))
            offset = 0
        locations[name] = (len(chunks) - 1, offset)
        chunks[-1][offset : offset + len(payload)] = payload
        offset += len(payload)
        logical_bytes += len(payload)
    return chunks, locations, logical_bytes
...
def build_fragment_assets(
    ...
    *,
    ...
    discarded_solid_fragments = 0
    discarded_solid_vertices = 0
    ...
    for inline_model, x_q3, y_q3, z_q3 in variant_ids:
        ...
        clipped = clip_brush_state_to_world(source, root, state)
        discarded_solid = tuple(
            fragment
            for fragment in clipped
            if source.leaves[fragment.leaf].contents == -2
        )
        discarded_solid_fragments += len(discarded_solid)
        discarded_solid_vertices += sum(
            len(fragment.polygon) for fragment in discarded_solid
        )
        fragments = [
            fragment
            for fragment in clipped
        ...
        while cursor < len(fragments):
            ...
            bucket_solid = source.leaves[leaf].contents == -2
            ...
            if not bucket_solid and parent == 0xFFFF:
                raise ValueError(f"non-solid brush leaf {leaf} has no world parent")
            buckets.extend(
                BUCKET_RECORD.pack(
                    leaf, first_fragment, count, int(bucket_solid), parent
                )
            )
            bucket_count += 1
            non_solid_bucket_count += int(not bucket_solid)
            if not bucket_solid:
                ...
                for vertex in fragment.polygon:
                    ...
                    if not bucket_solid:
                        for axis, coordinate in enumerate(vertex_q2):
                            bounds_min_q2[axis] = min(bounds_min_q2[axis], coordinate)
                            bounds_max_q2[axis] = max(bounds_max_q2[axis], coordinate)
                    if lightmapped:
            ...
            solid_fragments += count if bucket_solid else 0
            cursor = group_end
        ...
        variant_metadata.append(
            {
                ...
                "discardedSolidFragmentCount": len(discarded_solid),
    ...
    control_chunks, control_locations, control_used = _pack_control_tables(
        (
            ("variants", bytes(variant_directory)),
            ("faces", face_directory),
            ("buckets", bytes(buckets)),
        )
    )
    control_bank_count = len(control_chunks)
    ...
    descriptor_first_bank = control_bank_count
    vertex_first_bank = descriptor_first_bank + descriptor_bank_count
    bank_count = control_bank_count + descriptor_bank_count + vertex_bank_count
    ...
    header = FRAGMENT_HEADER.pack(
        ...
        len(source_face_ids),
        (control_locations["variants"][0] << 16) | control_locations["variants"][1],
        (control_locations["faces"][0] << 16) | control_locations["faces"][1],
        (control_locations["buckets"][0] << 16) | control_locations["buckets"][1],
        control_used,
    )
    control_chunks[0][: FRAGMENT_HEADER.size] = header
    chunks, checked_descriptor_banks, checked_vertex_banks = _fragment_banks(
        tuple(bytes(chunk) for chunk in control_chunks),
        bytes(descriptors),
        bytes(vertices),
    )
    if (
    ...
    metadata = {
        ...
        "controlBankCount": control_bank_count,
        "controlPaddedBytes": control_bank_count * ROM_BANK_BYTES,
        ...
        "leafDomain": {
            ...
            "discardedSolidFragmentCount": discarded_solid_fragments,
            "discardedSolidVertexOccurrenceCount": discarded_solid_vertices,
        ...
        "bankRoles": {
            "control": {
                "firstBank": first_bank,
                "bankCount": control_bank_count,
                "logicalBytes": control_used,
                "variants": {
                    "bank": first_bank + control_locations["variants"][0],
                    "bankIndex": control_locations["variants"][0],
                    "offset": control_locations["variants"][1],
                    "address": 0x8000 + control_locations["variants"][1],
                },
                "faces": {
                    "bank": first_bank + control_locations["faces"][0],
                    "bankIndex": control_locations["faces"][0],
                    "offset": control_locations["faces"][1],
                    "address": 0x8000 + control_locations["faces"][1],
                },
                "buckets": {
                    "bank": first_bank + control_locations["buckets"][0],
                    "bankIndex": control_locations["buckets"][0],
                    "offset": control_locations["buckets"][1],
                    "address": 0x8000 + control_locations["buckets"][1],
                },
            },
            "descriptors": {
                "firstBank": first_bank + descriptor_first_bank,
                "bankCount": descriptor_bank_count,
                "bytes": len(descriptors),
                "sha256": hashlib.sha256(descriptors).hexdigest(),
            },
            "vertices": {
                "firstBank": first_bank + vertex_first_bank,
