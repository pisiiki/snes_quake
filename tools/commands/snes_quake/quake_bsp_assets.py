...
from snes_tooling.shared.generated_outputs import OwnedFileFamily  # noqa: E402
from quake_lightmap_packing import (  # noqa: E402, F401
    LIGHTMAP_BANK_MASK,
    LIGHTMAP_BYTES_PER_GROUP,
    LIGHTMAP_PHASE_SHIFT,
    LIGHTMAP_SAMPLE_BITS,
    LIGHTMAP_SAMPLES_PER_GROUP,
    ROM_BANK_BYTES,
    PackedLightmapBuild,
    PackedLightmapLocation,
    lightmap_location_sample_offset,
    pack_lightmap_payloads,
    pack_six_bit_samples,
    unpack_six_bit_samples,
)
...
def pvs_directory_layout(
    payload_bytes: int, directory_bytes: int, guard_bytes: int
) -> tuple[int, int]:
    """Place the shared directory/guard contract in the tail of PVS bank 18."""
    if min(payload_bytes, directory_bytes, guard_bytes) < 0:
        raise ValueError("negative face-PVS section size")
    directory = (payload_bytes + 7) & ~7
    guards = directory + directory_bytes
    if guards + guard_bytes > 0x8000:
        raise ValueError("PVS2 tail cannot hold the aligned face-PVS directory and guards")
    return 0x8000 + directory, 0x8000 + guards
...
WORLD_TEXTURE_COORD_ROM_BANKS = 3
WORLD_LIGHTMAP_ROM_BANKS = 4
WORLD_PVS_ROM_BANKS = 4
...
OWNED_OUTPUT_FAMILIES = (
    OwnedFileFamily(prefix="QuakeBSPWorld", suffix=".bin"),
    OwnedFileFamily(prefix="QuakeBSPDemoPrecise", suffix=".bin"),
    OwnedFileFamily(prefix="QuakeBSPCollision", suffix=".bin"),
)
...
def locate_quake_pak(explicit: Path | None) -> Path:
    if explicit is not None:
        ...
        return explicit
    environment = os.environ.get("QUAKE_PAK")
    if environment:
        configured = Path(environment).expanduser()
        if not configured.is_file():
            raise FileNotFoundError(
                f"QUAKE_PAK does not name a file: {configured}"
            )
        return configured
    candidates = (
...
def source_winding_normal_q6(
    source_normal: tuple[object, ...], side: int
) -> tuple[int, int, int]:
    """Quantize the BSP face's source winding normal to signed Q6.

    Quake's face orientation points toward visible space, while the surfedge
    winding used by the packed polygon points the other way. A packed Q2
    polygon whose Q6 normal differs from this value can rotate across the
    source half-space and must bypass the cheap plane-only backface cull.
    """

    direction = 1.0 if side else -1.0
    normal = tuple(
        round(float(source_normal[axis]) * direction * 64) for axis in range(3)
    )
    if not any(normal) or any(
        component < -64 or component > 64 for component in normal
    ):
        raise AssertionError(f"invalid source winding Q6 normal {normal}")
...
def face_texture_coordinates(
    polygon: list[tuple[float, float, float]],
    texture_record: tuple[object, ...],
) -> tuple[list[float], list[float]]:
    """Return the source texture coordinates for one BSP polygon."""
    ...
    return coordinates_s, coordinates_t


def _texture_repeat_offset(coordinates: list[float], texture_extent: int) -> int:
    """Choose a whole-repeat offset that keeps one Q4 axis signed."""

    if texture_extent <= 0:
        raise ValueError("texture extent must be positive")
    if not coordinates:
        raise ValueError("texture-coordinate polygon is empty")
    q4 = [round_half_away_from_zero(value * 16.0) for value in coordinates]
    period_q4 = texture_extent * 16
    minimum_repeat = math.ceil((max(q4) - 0x7FFF) / period_q4)
    maximum_repeat = math.floor((min(q4) + 0x8000) / period_q4)
    if minimum_repeat > maximum_repeat:
        raise ValueError(
            "face texture-coordinate axis cannot fit signed Q4 after "
            "whole-texture wrapping"
        )
    if minimum_repeat <= 0 <= maximum_repeat:
        repeat = 0
    elif minimum_repeat > 0:
        repeat = minimum_repeat
    else:
        repeat = maximum_repeat
    return repeat * texture_extent


def normalized_face_texture_coordinates(
    polygon: list[tuple[float, float, float]],
    texture_record: tuple[object, ...],
    texture: MipTexture,
) -> tuple[list[float], list[float], tuple[int, int]]:
    """Normalize by whole repeats without changing texture/lightmap sampling."""

    coordinates_s, coordinates_t = face_texture_coordinates(polygon, texture_record)
    offset_s = _texture_repeat_offset(coordinates_s, texture.width)
    offset_t = _texture_repeat_offset(coordinates_t, texture.height)
    return (
        [value - offset_s for value in coordinates_s],
        [value - offset_t for value in coordinates_t],
        (offset_s, offset_t),
    )
...
def validate_drawable_face_colors(
    face_records: bytes, *, label: str, record_bytes: int = FACE_RECORD_BYTES
) -> int:
    """Reject any drawable face that uses the reserved clear-color index."""
    if len(face_records) % record_bytes:
        raise AssertionError(f"{label} face stream is not record-aligned")

    drawable_faces = 0
    for offset in range(0, len(face_records), record_bytes):
        face_index = offset // record_bytes
...
def build_world_node_topology(
    ...
    reachable_indices = {
        index for index, parent in enumerate(parents) if parent != unseen
    }
    if reachable_indices != set(range(reachable)):
        raise ValueError(
            "world BSP nodes are not a contiguous prefix of the source node table"
        )
    encoded_parents = tuple(
        0xFFFF if parent < 0 else parent for parent in parents[:reachable]
    )
    return encoded_parents, tuple(owners), reachable, maximum_depth
...
def lightmap_natural_colormap(colormap: bytes) -> bytes:
    ...
    return colormap[:table_bytes]


def missing_lightmap_level(texture_name: str, *, sky_enabled: bool = True) -> int:
    """Return Quake's fallback row for one face without baked samples."""

    folded = texture_name.casefold()
    bypasses_lightmap = folded.startswith("*") or (
        sky_enabled and folded.startswith("sky")
    )
    ...
    *,
    ...
    for face_index, face in enumerate(faces):
        ...
        if not 0 <= texture_id < len(textures):
            raise ValueError(f"world face {face_index} has invalid texture")
        texture = textures[texture_id]
        coordinates_s, coordinates_t, texture_offset = (
            normalized_face_texture_coordinates(polygon, texture_record, texture)
        )
        ...
        if face.light_offset == -1:
            ...
            missing_level = missing_lightmap_level(
                texture_name, sky_enabled=sky_enabled
            )
            records.append(
                {
                    "face": face_index,
                    ...
                    "texture_coordinate_offset": list(texture_offset),
                    "styles": [],
                    "source_offset": -1,
                }
            )
            face_samples.append(None)
            continue
        ...
        face_samples.append(bytes(packed))

    packed_build = pack_lightmap_payloads(
        tuple(samples for samples in face_samples if samples is not None),
        first_bank=WORLD_LIGHTMAP_FIRST_GSU_ROM_BANK,
        maximum_bank_count=WORLD_LIGHTMAP_ROM_BANKS,
    )
    directory = bytearray()
    location_iterator = iter(packed_build.locations)
    for record, samples in zip(records, face_samples, strict=True):
        if samples is None:
            encoded_level = LIGHTMAP_ENCODED_LEVEL_BIAS + int(record["missing_level"])
        ...
        minimum_s, minimum_t = record["texture_min"]
        directory.extend(
            struct.pack(
                "<BHBBbb",
                location.encoded_bank,
                location.address,
                width,
                height,
                minimum_s // 16,
        ...
        record["gsu_rom_bank"] = location.bank
        record["gsu_rom_address"] = location.address
        record["packed_phase"] = location.phase
        record["encoded_gsu_rom_bank"] = location.encoded_bank
    try:
        next(location_iterator)
    except StopIteration:
        pass
    else:
        raise AssertionError("packed lightmap location count exceeds face count")
    ...
    return bytes(directory), packed_build.chunks, records, packed_build
...
def world_table_include(info: dict[str, object]) -> bytes:
    ...
    lines = [
        ...
        f"BSP_WORLD_MAX_NODE_DEPTH = {info['max_node_depth']}",
        ...
        f"BSP_WORLD_FACE_RECORD_BYTES = {WORLD_FACE_RECORD_BYTES}",
        f"BSP_WORLD_FACE_PLANE_RECORD_BYTES = {FACE_PLANE_RECORD_BYTES}",
        f"BSP_WORLD_FACE_FLAG_NON_DRAWABLE = {WORLD_FACE_FLAG_NON_DRAWABLE}",
        f"BSP_WORLD_FACE_FLAG_PLANE_CULL_GUARD = {WORLD_FACE_FLAG_PLANE_CULL_GUARD}",
        ...
        f"BSP_WORLD_TEXTURE_FLAG_TURBULENT = {TEXTURE_FLAG_TURBULENT}",
        ...
        "BSP_WORLD_LIGHTMAP_SAMPLE_BITS = 6",
        "BSP_WORLD_LIGHTMAP_SAMPLES_PER_GROUP = 4",
        "BSP_WORLD_LIGHTMAP_BYTES_PER_GROUP = 3",
        f"BSP_WORLD_LIGHTMAP_LOGICAL_SAMPLE_COUNT = {info['lightmap_logical_sample_count']}",
        f"BSP_WORLD_LIGHTMAP_PACKED_BYTES = {info['lightmap_packed_bytes']}",
        ...
        f"BSP_WORLD_PVS_DIRECTORY_ROM_BANK = ${info['pvs_directory_rom_bank']:02X}",
        f"BSP_WORLD_PVS_DIRECTORY_ROM_ADDRESS = ${info['pvs_directory_rom_address']:04X}",
        ...
        f"BSP_WORLD_PVS_GUARD_ROM_BANK = ${info['pvs_directory_rom_bank']:02X}",
        f"BSP_WORLD_PVS_GUARD_ROM_ADDRESS = ${info['pvs_guard_rom_address']:04X}",
        ...
        (f"BSP_DEMO_PRECISE_SAMPLE_RATE_HZ = {info['demo_precise_sample_rate_hz']}"),
        f"BSP_DEMO_STEP_SAMPLE_RATE_HZ = {DEMO_STEP_SAMPLE_RATE_HZ}",
        f"BSP_DEMO_ORDERED_POSE_STRIDE = {info['demo_ordered_pose_stride']}",
...
def require_matching_demo_artifacts(track: Path, brush_replay: Path) -> int:
    """Return the bound camera hash or reject a mismatched QBR2 artifact pair."""
    track_bytes = track.read_bytes()
    replay_bytes = brush_replay.read_bytes()
    if len(replay_bytes) < 20 or replay_bytes[:4] != b"QBR2":
        raise ValueError(f"brush replay is not a QBR2 stream: {brush_replay}")
    bound_hash = struct.unpack_from("<Q", replay_bytes, 12)[0]
    actual_hash = 0xCBF29CE484222325
    for byte in track_bytes:
        actual_hash = ((actual_hash ^ byte) * 0x100000001B3) & 0xFFFFFFFFFFFFFFFF
    if actual_hash != bound_hash:
        raise ValueError(
            "configured camera track and brush replay do not match: "
            f"track={actual_hash:016x}, replay={bound_hash:016x}"
        )
    return actual_hash
